...
Reaction roles reconcile existing reactions and preserve pre-existing assignments. Only bot-owned role assignments are removed on opt-out. See [operations](operations.md) for edge cases and setup.
...
- High-level, rate-limited progress updates: queued, reasoning, calculating, searching, checking. Do not expose hidden reasoning.
...
Planned Oracle ARM/Ampere VPS: 3 CPU cores, approximately 23 GB RAM and 200 GB storage. Quality over speed. No model is selected: benchmark candidate reasoning and vision models on the actual VPS for answer quality, RAM, context limits and concurrency before choosing. Do not assume any VPS, model, Discord application, PM2 process or tunnel exists.
