...
The static allowlist is a comma-separated list of Discord user IDs. Whitespace and duplicate entries are handled; malformed IDs stop startup rather than broadening access. Static IDs merge with persistent owner-managed `/ai-testers` entries; both empty denies everyone. Add static IDs here or use the owner-only runtime command without editing environment files. This variable belongs in the bot configuration, not `.env.ai.*`. See [admin tools](admin-tools.md) for storage, precedence, audits and runtime commands.

The command registry enforces tester membership before every private command handler. The shared study submission helper also checks it, and the message handler checks membership and `MESSAGE_FEATURES_ENABLED` before any conversation lookup or inference request. Moderator/admin/owner status and coach roles never bypass membership; `/ai` still additionally requires moderator authorization (configured moderator role, creator or Manage Server). Non-testers receive an ephemeral slash-command rejection and their AI mentions/replies are silently ignored. Moderation continues independently for everyone's messages.
...
4. Apply tester-specific command overrides in each server as described above. Runtime `/ai-testers` changes apply on the next admission check with no restart or re-registration. Environment edits should follow the normal manual configuration/restart procedure. New non-admin testers still need Discord UI overrides.
