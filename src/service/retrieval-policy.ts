...
export function retrievalPolicy(prompt:string):RetrievalPolicy {
  ...
  const math=/\b(solve|calculate|equation|integer|polynomial|prove|proof|pairs|pigeonhole|stuffed toys|differentiate|integrate|sum|product|difference|quotient|plus|minus|times|multiplied|divided)\b/i.test(prompt);
  ...
  const required=explicit||current||(!math&&(exhaustive||(!safe&&factual)));
