...
export class DiscordBroker {
  ...
  request(job:Job,tool:DiscordTool,args:Record<string,unknown>,signal:AbortSignal):Promise<unknown>{
    ...
    return new Promise((resolve,reject)=>{
      ...
      this.tasks.set(id,{task:{id,job:job.id,user:job.user,guild:job.guild,channel:job.channel,tool,args},claimed:false,resolve,reject,clean});
  ...
  finish(id:string,value:unknown){const pending=this.tasks.get(id);if(!pending?.claimed)return false;pending.clean();pending.resolve(value);return true;}
