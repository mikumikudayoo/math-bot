...
export function runner(config:ServiceConfig,store:Store,dependencies:Partial<InferenceDependencies>={}) {
  ...
  return async (job:Job,signal:AbortSignal,status:(s:string)=>void):Promise<Result> => {
    ...
    const publishEvidence=()=>{
      const content=JSON.stringify({type:'RETRIEVED_EVIDENCE',originalQuestion:job.prompt,exactEntities:policy.entities,exhaustive:policy.exhaustive,sources:evidence});
    ...
    for(let round=0;round<6;round++){
      ...
      if(!calls.length){
        ...
        if(envelope&&typeof envelope.answer!=='string')throw new UserError('The model returned an unsupported response format.');
