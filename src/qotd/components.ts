...
export function answerModal(token: string) {
  return new ModalBuilder().setCustomId(`qotd:submit:${token}`).setTitle('Submit QOTD Answer').addComponents(
...
export async function handleQotdComponent(i: Interaction, competition: Competition, allowedGuild?: string) {
  ...
  try {
    if (!i.guildId || !i.channelId || (allowedGuild && i.guildId !== allowedGuild)) throw new Error('Use this in the QOTD server.');
    if (i.isButton()) {
      const match = /^qotd:answer:([1-9]\d*)$/.exec(i.customId);
      if (!match) throw new Error('Invalid QOTD button.');
      ...
      if (!match) throw new Error('Invalid QOTD modal.');
